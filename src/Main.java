...
public class Main {
    private static AnimalMenu aMenu;
    private static Scanner in;

    public static void main(String[] args) {
        aMenu = new AnimalMenu();
        in = new Scanner(System.in);
        while (true) {
            printOperations();
            execOperation(getInt("Выберете пункт меню: "));
            getText("Нажмите кнопку 'Enter' для продолжения");
        }
    }
    ...
    public static void printOperations() {
        ...
        System.out.println("4. Добавить нового животного");
        System.out.println("5. Обучить новым командам");
    ...
    protected static Animal getAnimalIndex() {
        while (true) {
            try {
                return aMenu.getRegister().get(getInt("Введите индекс животного: ") - 1);
            } catch (Exception e) {
    ...
    public static void printRegister() {
        ...
        System.out.println();
        for (Animal animal : aMenu.getRegister()) {
            index += 1;
